...
public abstract class XWrapper(XElement element) : INotifyPropertyChanged
{
    ...
    protected void SetInt(XElement? parent, string name, int val) =>
        SetElementValue(parent, name, SafeCast.Format(val));

    protected static int GetIntAttr(XElement? el, string name, int def = 0) => GetAttr(el, name).ToInt(def);
    protected void SetIntAttr(XElement? el, string name, int val) => SetAttr(el, name, SafeCast.Format(val));
    ...
    protected void SetLorId(XElement? parent, string tagName, LorId value)
    {
        ...
        {
            // 如果值为空，且节点不存在，就不创建了
            if (string.IsNullOrEmpty(value.ItemId)) return;
            node = new XElement(tagName);
            parent.Add(node);
        }

        node.Value = value.ItemId;

        if (value.PackageId != GlobalId.PackageId && !string.IsNullOrEmpty(value.PackageId))
        {
            node.SetAttributeValue("Pid", value.PackageId);
        }
        else
        {
